...
void DenseImageDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (this->layer_param_.dense_image_data_param().mirror()) {
      ...
      if (do_mirror) {
        cv::flip(cv_img,cv_img,-1); //flip = 0 for vertical flipping ,flip = -1 for flipping around both axes
        cv::flip(cv_lab,cv_lab,-1);
      }
    }
    // Apply random contrast and brightness change
  if (this->layer_param_.dense_image_data_param().contrast_adjust()) {
    float alpha = 1, beta = 0;
    const int apply_contrast = caffe_rng_rand() % 4;
    if ( apply_contrast > 2 ) {
     // float min_alpha = 0.8, max_alpha = 1.2;
      alpha = 0.8 + 0.1* (caffe_rng_rand() % 5);
